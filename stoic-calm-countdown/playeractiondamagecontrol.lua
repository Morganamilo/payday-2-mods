-- Everything is local so replace the entire file
-- Would be better to wrap the functions but that does not seem possible
...
PlayerAction.DamageControl.Function = function ()
	...
	while true do
		...
		if auto_shrug_time then
			Hooks:Call("check_calm_time", auto_shrug_time - now)

			if auto_shrug_time <= now then
				shrug_off_damage()
			end
		else
			Hooks:Call("check_calm_time", 0)
		end
	end
end
